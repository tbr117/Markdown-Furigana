...
export function applyStyles(settings: FuriganaProSettings): void {
  ...
  b.style.setProperty(
    "--furigana-font",
    settings.furiganaFontFamily || "inherit",
  );
  ...
  b.style.setProperty(
    "--furigana-base-font",
    settings.baseFontFamily || "inherit",
  );
}
...
export class FuriganaSettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    previewWrap.createSpan({
      cls: "furigana-settings-preview-label",
      text: "Preview:",
    });
    ...
    new Setting(containerEl)
      ...
      .addToggle((t) =>
        t
          .setValue(this.plugin.settings.showInLivePreview)
          .onChange(async (v) => {
    ...
    const sizeSetting = new Setting(containerEl)
      ...
      .setDesc(
        `Size relative to the base text. Current: ${this.plugin.settings.furiganaFontSize.toFixed(2)}em`,
      );
    sizeSetting.addSlider((s) =>
      s
        .setLimits(0.3, 1.2, 0.05)
        .setValue(this.plugin.settings.furiganaFontSize)
        .setDynamicTooltip()
        .onChange(async (v) => {
          this.plugin.settings.furiganaFontSize = v;
          sizeSetting.setDesc(
            `Size relative to the base text. Current: ${v.toFixed(2)}em`,
          );
          await this.plugin.saveSettings();
    ...
    const opacitySetting = new Setting(containerEl)
      ...
      .setDesc(
        `Opacity when visible. Current: ${Math.round(this.plugin.settings.furiganaOpacity * 100)}%`,
      );
    opacitySetting.addSlider((s) =>
      s
        .setLimits(0, 1, 0.05)
        .setValue(this.plugin.settings.furiganaOpacity)
        .setDynamicTooltip()
        .onChange(async (v) => {
          this.plugin.settings.furiganaOpacity = v;
          opacitySetting.setDesc(
            `Opacity when visible. Current: ${Math.round(v * 100)}%`,
          );
          await this.plugin.saveSettings();
          applyStyles(this.plugin.settings);
        }),
    ...
    new Setting(containerEl)
      ...
      .addColorPicker((p) =>
        p
          .setValue(this.plugin.settings.furiganaColor || "#888888")
          .onChange(async (v) => {
      ...
      .addExtraButton((b) =>
        b
          .setIcon("reset")
          .setTooltip("Reset — inherit from theme")
          .onClick(async () => {
    ...
    new Setting(containerEl)
      ...
      .addText((t) =>
        t
          .setPlaceholder('"Noto Sans JP", sans-serif')
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Color of the kanji / base text. Click ↺ to inherit from the theme.",
      )
      .addColorPicker((p) =>
        p
          .setValue(this.plugin.settings.baseColor || "#000000")
          .onChange(async (v) => {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "CSS font-family for the kanji / base text. Leave empty to inherit.",
      )
      .addText((t) =>
        t
          .setPlaceholder('"Noto Serif JP", serif')
    ...
    new Setting(containerEl)
      ...
      .addToggle((t) =>
        t
          .setValue(this.plugin.settings.autoReadingEnabled)
          .onChange(async (v) => {
    ...
    // Syntax table
    howto.createEl("p", {
      text: "Write furigana manually using curly-bracket syntax:",
    });
    ...
    const rows: [string, string][] = [
      ...
      ["{漢|かん|じ}", "Per-character annotation"],
